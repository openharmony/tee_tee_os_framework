...
static TEE_Result TestGetSecureRtcTime(TEE_Param params[4])
{
    ...
    uint32_t seconds = tee_get_secure_rtc_time();
    if (seconds == 0) {
        tloge("tee_get_secure_rtc_time failed, get secure rtc time is zero.\n");
        return TEE_ERROR_TIME_NEEDS_RESET;
    }
    params[1].value.a = seconds;
    tlogi("call tee_get_secure_rtc_time success! seconds = %d", seconds);
...
#define TEST_TYPE 1613  // set this value to avoid conflict with other timer,this type value should globally unique
static TEE_Result TestCreateDestoryExpireRemainTimer(TEE_Param params[4])
{
    tlogi("[%s] begin:", __FUNCTION__);

    TEE_Result ret = TEE_SUCCESS;

    TEE_timer_property timer_property = { 0 };
    uint32_t time_seconds = params[1].value.b;  // CA set Timer duration
    uint32_t time_seconds_ex, time_seconds_re;
    timer_property.type = TEST_TYPE;
    timer_property.timer_class = 3;

    ret = tee_ext_create_timer(time_seconds, &timer_property);
    if (ret != TEE_SUCCESS) {
        tloge("tee_ext_create_timer failed! ret = 0x%x\n", ret);
        return ret;
    }
    tlogi("tee_ext_create_timer success! timer_id = %d\n", timer_property.timer_id);

    const uint32_t waitTime = 500;
    TEE_Wait(waitTime);
    ret = tee_ext_get_timer_expire(&timer_property, &time_seconds_ex);
    if (ret != TEE_SUCCESS) {
        tloge("tee_ext_get_timer_expire failed! ret = 0x%x\n", ret);
        return ret;
    }
    params[1].value.a = time_seconds_ex;
    tlogi("tee_ext_get_timer_expire success! expire time = %d\n", time_seconds_ex);

    ret = tee_ext_get_timer_remain(&timer_property, &time_seconds_re);
    if (ret != TEE_SUCCESS) {
        tloge("tee_ext_get_timer_remain failed! ret = 0x%x\n", ret);
        return ret;
    }
    params[1].value.b = time_seconds_re;
    tlogi("tee_ext_get_timer_remain success! remain time = %d\n", time_seconds_re);

    ret = tee_ext_destory_timer(&timer_property);
    if (ret != TEE_SUCCESS) {
        tloge("tee_ext_destory_timer failed! ret = 0x%x\n", ret);
        return ret;
    }
    tlogi("tee_ext_destory_timer success!");
...
TEE_Result TestTimeApi(uint32_t cmdId, TEE_Param params[4])
{
    ...
    switch (cmdId) {
        ...
        case CMD_ID_TEST_GET_REE_TIME:
            ...
            break;
        case CMD_ID_TEST_GET_REE_TIME_STR:
            ret = TestGetREETimeStr(params);
            break;
        case CMD_ID_TEST_GET_SECURE_RTC_TIME:
            ret = TestGetSecureRtcTime(params);
            break;
        case CMD_ID_TEST_OPERATE_TIMER:
            ret = TestCreateDestoryExpireRemainTimer(params);
            break;
