...
#define Delta 1.0E-6
    ...
    d1 = strtod(c, &end);
    d2 = strtod(end, NULL);
    if (fabs(d1 - 365.24) > Delta || fabs(d2 - 29.53) > Delta) {
        printf("Failed: strtod(%s, X) got two double are %f and %f\n", c, d1, d2);
        return -1;
    }
    int i = abs(-10);
    if (i != 10) {
        printf("Failed: abs(-10) got %d\n", i);
        return -1;
    }
    long l = labs(-12345678);
    if (l != 12345678) {
        printf("Failed: labs(-12345678) got %ld\n", l);
        return -1;
    }
    long long ll = llabs(-12345678999l);
    if (ll != 12345678999l) {
    ...
    i = atoi("124");
    if (i != 124) {
        printf("Failed: atoi(\"124\") got: %d\n", i);
        return -1;
    }
    l = atol("12345678");
    if (l != 12345678) {
        printf("Failed: atol(\"12345678\") got: %ld\n", l);
        return -1;
    }
    ll = atoll("12345678999");
    if (ll != 12345678999l) {
        printf("Failed: atoll(\"12345678999\") got: %lld\n", ll);
        return -1;
    }
    div_t res = div(5, 3);
    if (res.quot != 1 || res.rem != 2) {
        printf("Failed: div(5, 3) got: quot=%d, rem=%d\n", res.quot, res.rem);
        return -1;
    }
    int d, n;
    char *str;
    str = ecvt(12.3, 5, &d, &n);
    if (strcmp(str, "12300") != 0 || d != 2 || n != 0) {
        printf("Failed: ecvt(12.3, 5, &d, &n) returns: %s, d=%d, n=%d\n", str, d, n);
        return -1;
    }
    double test_a = 23.12000000;
    printf("test_a: %lf\n", test_a);

//todo
    // const wchar_t *wstod = L"12.12 23.12";
    // wchar_t *wend;
    // double dd1 = wcstod(wstod, &wend);
    // printf("dd1 size:%zd\n",sizeof(dd1));
    // printf("double size:%zd\n",sizeof(double));
    // printf("wend: %ls\n", wend);
    // double dd2 = wcstod(wend, NULL);
    // printf("dd1: %lf\n", dd1);
    // printf("dd2: %lf\n", dd2);
    // if (fabs(d1 - 12.12) > Delta || fabs(d2 - 23.12) > Delta) {
    //     printf("Failed: wcstod(L\"12.12 23.12\", X) got two double are %lf and %lf\n", d1,
    //            d2);
    //     return -1;
    // }
    ...
    // qsort will sort 'months' by name through compmi method
    qsort(months, nr_of_months, sizeof(struct mi), compmi);
    // after sort "apr" will be the first.
    if (months[0].nr != 4) {
        printf("Failed: after qsort(months, ...), first month should be 4, but here got %d\n",
               months[0].nr);
        return -1;
    }
    struct mi key, *result;
    key.name = "aug";
    result = (struct mi *)bsearch(&key, months, nr_of_months, sizeof(struct mi),
                      compmi);
    if (result == NULL || result->nr != 8) {
        if (result == NULL)
            printf("Failed: after bsearch(&key, months, ...), result should be 8, but got NULL\n");
        else
            printf("Failed: after bsearch(&key, months, ...), result should be 8, but got %d\n",
                   result->nr);
        return -1;
    }
    printf("=== test stdlib end   ===\n\n");
#undef Delta
    return 0;
}
...
{
    char *tst_getenv;
    tst_getenv = getenv("NOT_EXIST");
    if (tst_getenv != NULL) return -2;
